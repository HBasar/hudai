...
user* InitializeList(user *h)
{
    ...
    rewind(fp);
	while(fscanf(fp,"%s %s %s %f %d",temp.username,temp.password,temp.place,&temp.price,&temp.numticket)!=EOF)
	{
...
void WriteToFile(user *h)
{
    ...
    while(h!=NULL)
    {
        fprintf(fp,"%s %s %s %f %d\n",h->username,h->password,h->place,h->price,h->numticket);
        h=h->next;
    }
    fclose(fp);
}
...
{
	system("CLS");
    printf("\tPRICE LIST\n=============================\n1. BB - Bandarban Tour Package - TK 4000\n2. SJK - Sajek  Tour Package - TK 6000\n3. CB - Cox's Bazar Tour Package - TK 10000\n4. SM - Saint Martin Tour Package - TK 3800\n"
           "5. SK - Sikim Tour Package - TK 50000\n6. TL - Thailand Tour Package - TK 200000\n7. SP - Singapore Tour Package - TK 300000\n");
}
...
void BookTicket(user *h)
{
    ...
    ShowBrochure();
    float pricelist[]={4000.0,6000.0,10000.0,3800.0,50000.0,200000.0,300000.0};
    fflush(stdin);
    printf("\nEnter place code (eg: BB, CB)\n");
    scanf(" %[^\n]s",place);
    char choice;
    fflush(stdin);
    ...
    if(choice!='1')
        return;
    if(strcmp(place,"BB")==0)
        price=pricelist[0];
    else if(strcmp(place,"SJK")==0)
        price=pricelist[1];
    else if(strcmp(place,"CB")==0)
        price=pricelist[2];
    else if(strcmp(place,"SM")==0)
        price=pricelist[3];
    else if(strcmp(place,"SK")==0)
        price=pricelist[4];
    else if(strcmp(place,"TL")==0)
        price=pricelist[5];
    else if(strcmp(place,"SP")==0)
        price=pricelist[6];
...
void PrintTicket(user *h)
{
    ...
    if(fgetc(fp)==EOF)
    {
        fprintf(fp,"TOUR TICKET\n===============\n\n");
    }
    fprintf(fp,"Email ID: %s\nTour Code: %s\nTicket Cost: TK %f\nNumber of tickets: %d\nTotal Cost: TK %f\n",h->username,h->place,h->price,h->numticket,total);
    fclose(fp);
}
...
void CancelTicket(user *h)
{
    ...
    if(strcmp(h->place,"BB")==0)
        flag++;
    else if(strcmp(h->place,"SJK")==0)
        flag++;
    else if(strcmp(h->place,"CB")==0)
        flag++;
    else if(strcmp(h->place,"SM")==0)
        flag++;
    else if(strcmp(h->place,"SK")==0)
        flag++;
    else if(strcmp(h->place,"TL")==0)
        flag++;
    else if(strcmp(h->place,"SP")==0)
        flag++;
...
void ExitProgram()
{
    printf("Exiting...\nDeveloped by Nazmul and Razin and Biplob, Bangladesh\n\nBrought To You By code-projects.org\n\nPress \"Enter/Return\" to exit");
    char exitprog;
    fflush(stdin);
    scanf("%c",&exitprog);
}
